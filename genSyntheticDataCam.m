...
for n=1:numSamples
    ...
    fprintf(fileIDData, formatSpecCam, camData);
...
function [camData, lidarData] = makeCamLidarData(xCurr)


posNED = xCurr(1:3,1);
nQb = xCurr(7:10,1);
nRb = quat2dcm(nQb'); bRn = nRb';
posTargetNED = [200, 150, 300]'; cRb = eye(3);
posTargetBody = bRn * (posTargetNED - posNED); %Target postion in body frame of reference
posTargetCamLidar = cRb * posTargetBody; %Target position in Camera/Lidar frame of reference
posTargetPolarLidar = [atan2(posTargetCamLidar(1), posTargetCamLidar(2)); ...
                  atan2(posTargetCamLidar(3), sqrt(posTargetCamLidar(1)^2 + posTargetCamLidar(2)^2)); ...
                  norm(posTargetCamLidar)]; %[Azimuth, Elevation, Range]'. For Lidar
%Lidar Sensor in XZ Plane of body axis. No translation between Lidar axis and body axis.
             
posTargetProjectionCam = [posTargetCamLidar(1)/posTargetCamLidar(3); posTargetCamLidar(2)/posTargetCamLidar(3)];
%Camera Sensor in XY Plane of body axis. No Tanslation between camera axis and body axis

camData = posTargetProjectionCam'; %Target projection at camera sensor. Sensor origin matches with body axis origin.
lidarData = posTargetPolarLidar';
end
%%
