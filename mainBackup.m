...
for m = 1:1:M
    ...
    stdIni = 0*[2.55, 0.051, 0.5, 0, 0]'; % std for Error in initial guess, %stdIni -> [Pos, Vel, euler, biasW, biasA];
...
%         H(7:9,7:10)
        ...
        if (m == 1)
            ...
            if true   %For testing only
                ...
                aTrueNEDPre(:,n) = nRb * aBody;
                if n>1
                    vTrueNED(:,n) = vTrueNED(:,n-1) + (aTrueNEDPre(:,n)-[0 0 -9.80665]') * dtTrue(n);
                    pTrueNED(:,n) = pTrueNED(:,n-1) + (vTrueNED(:,n-1)) * dtTrue(n);
%                     qTrueNED(:,n-1) = rot2quat(nRb);
                
                else
                    vTrueNED(:,n) = zTrueIni(4:6) + (aTrueNEDPre(:,n)-[0 0 -9.80665]') * dtTrue(n);  ;%zTrue(4:6,1);
                    pTrueNED(:,n) = zTrueIni(1:3) + (zTrueIni(4:6)) * dtTrue(n);%zTrue(1:3,1);
%                     qTrueNED(:,n) = quat2euler(zTrueIni(7:9));
                end
...
% misc(pTrueNED-xTrueComp(1:3, 2:end), time, 1:3); suptitle('Linear Position by dead reckoning');
% misc(vTrueNED-xTrueComp(4:6, 2:end), time, 1:3); suptitle('Velocity Error by dead reckoning');
end
